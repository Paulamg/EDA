...
##Graphic 1
with(dates, {
	plot(cd, Global_active_power, pch="", ylab="Global Active Power (kilowattz)", xlab="")
	lines(dates$cd, dates$Global_active_power)
##Graphic 2
plot(cd, Voltage, pch="", ylab="Voltage", xlab="Datetime")
lines(dates$cd, dates$Voltage)
##Graphic 3
plot(cd, Sub_metering_1, type="n", ylab="Energy sub metering", xlab="", cex=0.8)
...
##Graphic 4
plot(cd, Global_reactive_power, pch="", xlab="datetime")
lines(dates$cd, dates$Global_reactive_power)
} )
